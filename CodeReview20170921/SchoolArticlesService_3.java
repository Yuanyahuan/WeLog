...
public class SchoolArticlesService extends AppGlobalService {
    ...
    private void buildArticlesListData(SchoolArticleDTO dto, List<Map<String, Object>> list) throws Exception {
        ...
        // 此处判断条件可以改为Switch Case，如果还有可能会增加可以采用装填模式或者责任链模式进行解决
        if (dto.getCategoryId() != 0) {
            ...
            if (!StringUtil.isEmpty(SchoolArticleCategoryEnum.getMsgByCode(dto.getCategoryId()))) {
                ...
                if (dto.getQueryType() == 0 || dto.getQueryType() == 20) {
                    ...
                    List<SjtSchoolArticleEntity> schoolArticleEntityList = schoolArticleComp
                            .getListByPage(sqls.getCondition(), 1, 10, "sort_id desc", sqls.getParams());
                    ...
                    List<SjtSchoolArticleEntity> schoolArticleEntityList = schoolArticleComp
                            .getListByPage(sqls.getCondition(), 1, 10, "num desc,id asc", sqls.getParams());
                    if (CollectionUtils.isNotEmpty(schoolArticleEntityList)) {
                        for (SjtSchoolArticleEntity schoolArticleEntity : schoolArticleEntityList) {
                            Map<String, Object> map = wrap(schoolArticleEntity, dto);
                            list.add(map);
                    ...
                    List<SjtSchoolArticleEntity> schoolArticleEntityList = schoolArticleComp
                            .getListByPage(sqls.getCondition(), 1, 10, "sort_id asc", sqls.getParams());
                    if (CollectionUtils.isNotEmpty(schoolArticleEntityList)) {
                        for (SjtSchoolArticleEntity schoolArticleEntity : schoolArticleEntityList) {
                            Map<String, Object> map = wrap(schoolArticleEntity, dto);
                            list.add(map);
            ...
            //lastId<0代表下拉刷新；  0代表默认值；  >0代表上拉加载
            if (sortId > 0) {
                sqls.conLittleThan("sort_id", sortId);
            }
            List<SjtSchoolArticleEntity> schoolArticleEntityList = schoolArticleComp.getListByPage(sqls.getCondition(),
                    1, 10, "sort_id desc", sqls.getParams());
            if (CollectionUtils.isNotEmpty(schoolArticleEntityList)) {
                for (SjtSchoolArticleEntity schoolArticleEntity : schoolArticleEntityList) {
                    Map<String, Object> map = wrap(schoolArticleEntity, dto);
                    list.add(map);
    ...
    private Map<String, Object> wrap(SjtSchoolArticleEntity schoolArticleEntity, SchoolArticleDTO dto)
            throws Exception {
        ...
        String pic = schoolArticleEntity.getPic() == null ? ""
                : fileComp.picUrlByUserId(schoolArticleEntity.getPic(), dto.getUserId());
        map.put("pic", pic);
